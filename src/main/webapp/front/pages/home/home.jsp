...
<body>
  <div id="app">
    ...
    <div class="part2" style="margin-top: 50px;">
      <div class="part2_1">
        ...
        <div class="more">
            <a href="#" @click="jump('../xinwenxinxi/list.jsp')">MORE>></a>
        </div>
      </div>
      <div class="part2_2">
        <div class="caseBox" v-for="(item,index) in xinwenxinxiList" v-bind:key="index">
          <div class="caseBox_1">
            ...
            <div class="hoverbg" style="display: none;">
                <a :href="'../xinwenxinxi/detail.jsp?id='+item.id">查看详情</a>
            </div>
          </div>
          <div class="caseBox_2">
            <a :href="'../xinwenxinxi/detail.jsp?id='+item.id">{{item.biaoti}}<span style="color: red;float: right;" v-if="item.price">{{item.price}} RMB</span></a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="part2" style="margin-top: 50px;">
      <div class="part2_1">
        ...
        <div class="more">
            <a href="#" @click="jump('../weiboxinxi/list.jsp')">MORE>></a>
        </div>
      </div>
      <div class="part2_2">
        <div class="caseBox" v-for="(item,index) in weiboxinxiList" v-bind:key="index">
          <div class="caseBox_1">
            <div class="caseImg" style="background-size: cover;background-repeat: no-repeat;background-position: center;"><img :src="item.tupian?item.tupian.split(',')[0]:''" width="390" height="320" /></div>
            <div class="hoverbg" style="display: none;">
                <a :href="'../weiboxinxi/detail.jsp?id='+item.id">查看详情</a>
            </div>
          </div>
          <div class="caseBox_2">
            <a :href="'../weiboxinxi/detail.jsp?id='+item.id">{{item.biaoti}}<span style="color: red;float: right;" v-if="item.price">{{item.price}} RMB</span></a>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery'], function() {
      ...
      http.request(`xinwenxinxi/list`, 'get', {
        page: 1,
        limit: 6
      }, function(res) {
        vue.xinwenxinxiList = res.data.list;
      });
      http.request(`weiboxinxi/list`, 'get', {
        page: 1,
        limit: 6
      }, function(res) {
        vue.weiboxinxiList = res.data.list;
      });
    });
  </script>
</body>
