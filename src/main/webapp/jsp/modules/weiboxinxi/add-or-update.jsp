...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑微博信息</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>微博信息管理</span></li>
								<li class="breadcrumb-item active"><span>编辑微博信息</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">微博信息信息</h3>
									...
									<div class="form-group col-md-6">
										...
										<input id="yonghuxingming" name="yonghuxingming" class="form-control" placeholder="用户姓名">
											...
											<div class="form-group  col-md-12">
												<label>微博内容</label>
												<div><div class="upload"><input id="weiboneirongupload" name="file" type="file">选择文件</div>
												<script id="weiboneirongEditor" type="text/plain" style="width:800px;height:230px;"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var weiboneirongUe = UE.getEditor('weiboneirongEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="weiboneirong" id="weiboneirong-input">
												</div>
											</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "weiboxinxi/info/" + id,
		...
		function upload(){
			...
			$('#weiboneirongupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('weiboneirongEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("weiboxinxi/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.weiboneirong != null && ruleForm.weiboneirong != 'null' && ruleForm.weiboneirong != ''){
				var weiboneirongMes = '' + ruleForm.weiboneirong;
				var weiboneirongUeditor = UE.getEditor('weiboneirongEditor');
				weiboneirongUeditor.ready(function() {
					weiboneirongUeditor.setContent(weiboneirongMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('weiboneirongEditor').hasContents()){
				$('#weiboneirong-input').attr('value',UE.getEditor('weiboneirongEditor').getPlainTxt());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("weiboxinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#weiboneirong").val(ruleForm.weiboneirong);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.weiboneirong != null && res.data.weiboneirong != ''){

					$("#weiboneirong").val(res.data.weiboneirong);
					}
